...
from graphgps.loader.dataset.GTeventlogHandler import (
    EVENTBPIC12, EVENTHelpDesk, EVENTSepsis, EVENTHospital, 
...
def join_dataset_splits(datasets):
    ...
    # Apply bucketing strategies
    if cfg.bucketing in {'C_B' , 'L_B', 'SSD_B'}:
        old_data_list = data_list
        if cfg.bucketing == 'C_B':
            data_list = [g for g in old_data_list if int(g.cluster_id) == cfg.segment]
        elif cfg.bucketing == 'SSD_B':
            data_list = [g for g in old_data_list if g.cid in cfg.segment]
        else:
            data_list = [g for g in old_data_list if g.pl in cfg.segment]
        # print('length of the list:', len(data_list))
        # remap split indices after filtering
        index_map = {id(g): new_i for new_i, g in enumerate(data_list)}
        # Original ranges
        train_range = range(n1)
        val_range   = range(n1, n1 + n2)
        test_range  = range(n1 + n2, n1 + n2 + n3)
        # Translate old indices → new indices ONLY if the graph survived filtering
        new_train = [index_map[id(old_data_list[i])] for i in train_range
                     if id(old_data_list[i]) in index_map]
        new_val   = [index_map[id(old_data_list[i])] for i in val_range 
                     if id(old_data_list[i]) in index_map]
        new_test  = [index_map[id(old_data_list[i])] for i in test_range 
                     if id(old_data_list[i]) in index_map]
        split_idxs = [new_train, new_val, new_test]
    else:
...
def preformat_EVENTHelpDesk(dataset_dir):
    ...
    if len(dataset.split_idxs[1]) == 0:  # validation set empty
        train_idxs = dataset.split_idxs[0]
        n_val = max(1, int(0.2 * len(train_idxs)))  # 20% of train
        random.shuffle(train_idxs)  # randomize
        val_idxs = train_idxs[:n_val]
        train_idxs = train_idxs[n_val:]
        dataset.split_idxs[0] = train_idxs
        dataset.split_idxs[1] = val_idxs
        logging.warning(f"Validation set was empty. Created {len(val_idxs)} samples from training data.")
    return dataset
